...
const form = useForm({
    confirmation: '', // Field to confirm by typing the project name
});

const deleteProject = () => {
    // Client-side validation: check if the typed confirmation matches the project name
    if (form.confirmation !== props.projectName) {
        form.setError('confirmation', 'The project name you entered does not match.');
        confirmInput.value?.focus();
        return;
    }

    form.delete(route('projects.destroy', { project: props.projectId }), {
        preserveScroll: true,
        onSuccess: () => {
            closeModal();
...
<template>
    <Dialog :open="dialogOpen" @update:open="dialogOpen = $event">
        ...
        <DialogContent>
            <form class="space-y-6" @submit.prevent="deleteProject">
                <DialogHeader class="space-y-3">
                    ...
                    <DialogDescription>
                        ...
                        <br />
                        <span class="font-bold text-red-600 dark:text-red-400">This will NOT delete the associated terrains.</span>
                        <br /><br />
                        Please type the project's name "<span class="font-semibold text-gray-900 dark:text-white">{{ projectName }}</span>" to confirm.
                    </DialogDescription>
                </DialogHeader>

                <div class="grid gap-2">
                    <Label for="confirmation" class="sr-only">Confirm Project Name</Label>
                    <Input
                        id="confirmation"
                        type="text"
                        name="confirmation"
                ...
                <DialogFooter class="gap-2">
                    ...
                    <Button type="submit" variant="destructive" :disabled="form.processing || form.confirmation !== projectName">
